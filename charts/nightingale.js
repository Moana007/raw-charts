...
//STYLE.CSS
	...
	document.write('<link rel="stylesheet" type="text/css" href="css/nightingale-original.css">'); 
...
(function(){
	...
	var dataTotal = model.dimension() 
		.title('Total d\'une part')
		.types(Number)

	// angle/ordre/centre (=date)
	var  dataAngle = model.dimension() 
		.title('Catégories et label (angle)')
		.types(String)
	...
	// -----------------
	var chart = raw.chart()
		.title("Nightingale Rose (Custum: data stack)")
		.description("Simple representation of an Nightingale Rose. The data representation stacks")
...
//3	
		...
		rose.legend(causes)
			...
			.delay(0)
			.domain([0, maxRadius])
			.angle(function(d){ return d.dataAngle.getMonth(); })
